...
from DASC500.stats.dashboard.utils.statistics import safe_statistical_test, handle_missing_values
    ...
    if advanced_transform:
        ...
        with col2:
            ...
            try:
                ...
                # Add option to add transformed data to the dataset
                if st.checkbox("Add transformed data to dataset for further analysis"):
                    new_col_name = f"{transform_col}_{advanced_transform}"
                    
                    # Add the transformed data to the dataframe
                    try:
                        df[new_col_name] = transformed_data
                        st.success(f"Successfully added '{new_col_name}' to the dataset.")
                    except ValueError as e:
                        st.error(f"Error adding transformed data to DataFrame: {e}")
                        st.info("This may be due to the data types not aligning. Ensure no NaNs are present.")
                    except Exception as e:
                        st.error(f"An unexpected error occurred: {e}")
                        
                    # Show message that it is available for analysis
                    st.info(f"'{new_col_name}' is now available for selection in the dashboard.")
    ...
    if advanced_transform:
        ...
        with col2:
            ...
            try:
                ...
                # Add option to add transformed data to the dataset
                if st.checkbox("Add transformed data to dataset for further analysis"):
                    session_key = f"transformed_{transform_col}_{advanced_transform}"
                    
                    if session_key not in st.session_state:
                        st.session_state[session_key] = transformed_data
                        st.success(f"Added transformed data as '{transform_col}_{advanced_transform}' to the dataset.")
                        
                        # Update the dataframe in session state
                        if 'df_with_transformations' not in st.session_state:
                            st.session_state.df_with_transformations = df.copy()
                        
                        st.session_state.df_with_transformations[f"{transform_col}_{advanced_transform}"] = transformed_data
                        
                        st.info("You can now select this transformed column for further analysis.")
                    else:
                        st.info(f"Transformed data '{transform_col}_{advanced_transform}' already exists in the dataset.")
