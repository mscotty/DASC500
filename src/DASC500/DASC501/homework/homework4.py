...
try:
    ...
    OUT_FOLDER = os.path.normpath(
        os.path.join(get_top_level_module_path(), r"..\..\outputs\DASC501\homework4")
    )
    ...
    with DatabaseManager(
        ...
        try:
            ...
            WHERE s.StudentID IN (
                -- This nested query selects the IDs of students who meet the contract criteria
                SELECT c.StudentID
                FROM tblContract c
            ...
            WHERE s.StudentID NOT IN (
                -- This nested query selects all distinct StudentIDs that *do* exist in the contract table
                SELECT DISTINCT c.StudentID
                FROM tblContract c
                WHERE c.StudentID IS NOT NULL -- Ensure we only get actual student IDs
            );"""
            run_and_print_query(
            ...
            SELECT
                JoinedData.City,
                MIN(JoinedData.Cost) AS MinCost,
                AVG(JoinedData.Cost) AS MeanCost,
                MAX(JoinedData.Cost) AS MaxCost
            FROM (
                -- This nested query joins the tables and filters relevant records first
                SELECT s.City, c.Cost
                FROM tblStudent s JOIN tblContract c ON s.StudentID = c.StudentID
                WHERE s.City IS NOT NULL AND c.Cost IS NOT NULL
            ) AS JoinedData -- Assign an alias to the subquery result
            GROUP BY JoinedData.City
            ORDER BY MeanCost ASC; -- Ordering by the calculated average cost"""
            run_and_print_query(
                db_manager,
