...
def plot_histogram(data,
    ...
    @param[in] bin_edges (array-like) Custom bin edges, if specified. Takes precedence over other binning options.
    ...
    # Check if bin_edges is provided, which takes precedence
    if bin_edges is not None:
        binning_meth = 'Custom'
        bin_width = None
        bin_count = len(bin_edges) - 1  # Number of bins is one less than number of edges
        print(f'Using custom bin edges with {bin_count} bins')
    elif binning_method is None and bin_width is None and bin_count is None:
    ...
    # Determine bin width or bin count if bin_edges is not provided
    if bin_edges is None:
        ...
        # Use the provided bin_edges
        bins = None  # We'll use xbins=None with the bin_edges parameter
        bin_str = 'custom_bins'
    ...
    if bin_edges is not None:
        # When custom bin edges are provided
        fig.add_trace(
            go.Histogram(
                x=data,
                autobinx=False,
                xbins=dict(
                    ...
                    size=(bin_edges[-1] - bin_edges[0]) / (len(bin_edges) - 1) if len(bin_edges) > 1 else 1
                ),
